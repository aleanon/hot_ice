    ...
    P::Message: Clone,
{
    ...
    pub fn new(program: &P) -> (Self, Task<Message<P>>) {
        ...
        let mut lib_reloader = LibReloader::new("target/debug/lib/debug", "ui", Some(Duration::from_millis(50)), None).expect("Unable to create LibReloader");
        let change_subscriber = lib_reloader.subscribe_to_file_changes();
        let mut map  = HashMap::new();
            map.insert("ui", Arc::new(Mutex::new(lib_reloader)));

        LIB_RELOADER.get_or_init(||map);
        ...
        std::thread::spawn(move || {                        
            loop {
                ...
                loop {
                    if let Some(map) = LIB_RELOADER.get() {
                        let Some(reloader) = map.get("ui") else {
                            println!("Library not found in map");
                            break;
                        };
                        if let Ok(mut lib_reloader) = reloader.lock() {
